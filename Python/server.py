...
import requests
import numpy as np
# url='http://192.168.17.15:8080/shot.jpg'
# while 1:
...
encoding="dulieu.pickle"
image="data_dat/datdata/th.jpg"
...
def encode():
    global step
    global total
    # encode_face("data_dat")
    dataset = "data_dat"
    encode = "dulieu.pickle"
    detection_method = "cnn"
  ...
  # đọc ảnh
        ...
        print(image.shape)
#chuyển ảnh dạng màu về dạng xám
...
def recog():
    image_b64 = request.form.get('image')
    data = pickle.loads(open("dulieu.pickle", "rb").read())
    image = np.fromstring(base64.b64decode(image_b64), dtype=np.uint8)
    image = cv2.imdecode(image, cv2.IMREAD_ANYCOLOR)
    print(image)
